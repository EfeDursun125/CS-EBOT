...
bool Bot::IsOnAttackDistance(edict_t* targetEntity, const float distance)
{
	if (FNullEnt(targetEntity))
		return false;
		...
		else
		{
			...
			else
			{
				if (m_followWaitTime + 5.0f < engine->GetTime())
				{
					...
					RadioMessage(Radio::YouTakePoint);
					TaskComplete();
...
bool Bot::CampingAllowed(void)
{
	...
	if (ebot_followuser.GetInt() > 0 && (m_radioOrder == Radio::FollowMe || GetCurrentTask()->taskID == TASK_CAMP))
	{
		int numFollowers = 0;
